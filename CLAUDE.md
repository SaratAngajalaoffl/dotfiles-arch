...
**Wallpapers are themes.** `theme/bin/wallpaper.py` manages a wallpaper library — every theme's `backgrounds/` plus `theme/config/wallpapers/`, named and tagged in `theme/config/wallpapers.json`. Applying a theme's own background uses its curated palette; any other image gets a palette generated by matugen (rendered through the same templates into `~/.local/state/theme/generated/<slug>/`, never committed). It then runs `theme-set.sh <dir>` with `WALLPAPER=<image>`, which also reloads open kitty windows (`SIGUSR1`). The shell's Themes widget is the UI for this; `wallpaper.py import <files> --tags a,b` adds images.

`~/.config/theme/current` is a symlink to the active theme directory (or a generated one), repointed by `theme-set.sh`. `install.sh` bootstraps the default theme automatically on a fresh install if no theme has been set yet — it never overrides an already-chosen theme on a subsequent run.
